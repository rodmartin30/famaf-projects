...
pair_t pair_from_index_data(index_t index, data_t data) {
	...
	assert(result != NULL && index_is_equal(result->first, index) && data_is_equal(result->second, data));
...
bool pair_is_equal(pair_t pair, pair_t other) {
	...
	return index_is_equal(pair->first, other->first) && data_is_equal(pair->second, other->second);
}
...
pair_t pair_copy(pair_t pair) {
	...
	result = pair_from_index_data(index_copy(pair->first), data_copy(pair->second));
