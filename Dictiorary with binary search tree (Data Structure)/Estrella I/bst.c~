...
bst_t bst_destroy(bst_t bst) {
    ...
    if (bst != NULL) {
        bst->root = pair_destroy(bst->root);
        if (bst->left != NULL) {
...
bool bst_is_equal(bst_t bst, bst_t other) {
    ...
    if (bst != NULL && other != NULL) {
        result = pair_is_equal(bst->root, other->root);
        if (result) {
...
data_t bst_search(bst_t bst, index_t index) {
    data_t data = NULL;
    if (bst != NULL) {
        if (index_is_equal(index, pair_fst(bst->root))) {
            data = pair_snd(bst->root);
        } else {
            if (index_is_less_than(index, pair_fst(bst->root))) {
...
bst_t bst_remove(bst_t bst, index_t index) {
    ...
    if (bst != NULL) {
        ...
        } else if (index_is_equal(pair_fst(bst->root), index) &&
                   bst->left == NULL) {
            current = bst->right;
            bst->root = pair_destroy(bst->root);
            free(bst);
...
bst_t bst_copy(bst_t bst) {
    ...
    if (bst != NULL) {
        result = bst_add(result, index_copy(pair_fst(bst->root)),
                         data_copy(pair_snd(bst->root)));
...
list_t bst_to_list(bst_t bst, list_t list) {
    if (bst != NULL) {
        index_t index = index_copy(pair_fst(bst->root));
        data_t data = data_copy(pair_snd(bst->root));
